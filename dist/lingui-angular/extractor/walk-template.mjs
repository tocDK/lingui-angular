import { AST, Binary, BindingPipe, Call, Conditional, Interpolation, KeyedRead, LiteralArray, LiteralMap, LiteralPrimitive, NonNullAssert, parseTemplate, PrefixNot, PropertyRead, SafeCall, SafeKeyedRead, SafePropertyRead, TmplAstBoundText, TmplAstDeferredBlock, TmplAstElement, TmplAstForLoopBlock, TmplAstIfBlock, TmplAstSwitchBlock, TmplAstTemplate, } from '@angular/compiler';
export function walkTemplate(source, filePath) {
    ...
    const walk = (nodes) => {
        for (const node of nodes) {
            if (node instanceof TmplAstElement || node instanceof TmplAstTemplate) {
                ...
                // bodies on `.children`.
                const sw = node;
                for (const g of sw.groups ?? sw.cases ?? [])
                    walk(g.children);
            }
            else if (node instanceof TmplAstDeferredBlock) {
                walk(node.children);
                if (node.placeholder)
                    walk(node.placeholder.children);
                if (node.loading)
                    walk(node.loading.children);
                if (node.error)
                    walk(node.error.children);
            }
        }
    };
    walk(parsed.nodes);
...
function handleBoundText(node, filePath, calls, warnings) {
    const ast = node.value.ast;
    if (!(ast instanceof Interpolation))
        return;
    // ParseLocation carries the full file content so we can convert offsets to line/col.
    const fileContent = node.sourceSpan.start.file?.content;
    for (const expr of ast.expressions) {
        if (!(expr instanceof BindingPipe))
            continue;
        if (expr.name !== 't' && expr.name !== 'tPlural' && expr.name !== 'tSelect')
            continue;
        ...
        const { line, column } = resolvePosition(expr.exp.sourceSpan.start, fileContent, node.sourceSpan.start);
        switch (expr.name) {
            case 't':
 ...
 * unavailable.
 */
function resolvePosition(absOffset, fileContent, fallback) {
...
function handleTPipe(pipe, filePath, line, column, calls, warnings) {
...
function handleRulesPipe(pipe, kind, filePath, line, column, calls, warnings) {
...
function handleBoundAttr(attr, filePath, calls, warnings) {
    const ast = attr.value.ast;
    ...
    const fileContent = attr.sourceSpan.start.file?.content;
 ...
 */
function visitForTPipes(ast, attr, filePath, fileContent, calls, warnings) {
    if (ast instanceof BindingPipe) {
        if (ast.name === 't' || ast.name === 'tPlural' || ast.name === 'tSelect') {
            const { line, column } = resolvePosition(ast.exp.sourceSpan.start, fileContent, { line: attr.sourceSpan.start.line, col: attr.sourceSpan.start.col });
            switch (ast.name) {
                case 't':
    ...
    if (ast instanceof Interpolation) {
        for (const expr of ast.expressions)
            visitForTPipes(expr, attr, filePath, fileContent, calls, warnings);
        return;
    }
    ...
    if (ast instanceof LiteralMap) {
        ...
        for (const value of ast.values) {
            if (value instanceof AST)
                visitForTPipes(value, attr, filePath, fileContent, calls, warnings);
        }
        return;
    }
    if (ast instanceof Call || ast instanceof SafeCall) {
        ...
        for (const arg of ast.args)
            visitForTPipes(arg, attr, filePath, fileContent, calls, warnings);
        return;
    }
...
function renderShim(calls, filePath) {
    const lines = [`import { plural, select, t } from '@lingui/core/macro';`];
    for (const call of calls) {
        lines.push(`// @source: ${filePath}:${call.line}:${call.column}`);
        if (call.comment)
            lines.push(`// ${call.comment}`);
        switch (call.kind) {
            case 't': {
                const desc = { message: call.message };
                if (call.context)
                    desc['context'] = call.context;
                if (call.id)
                    desc['id'] = call.id;
